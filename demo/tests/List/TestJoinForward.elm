module List.TestJoinForward exposing (..)

import Array
import Expect
import RichText.Definitions exposing (doc, listItem, orderedList, paragraph)
import RichText.List exposing (defaultListDefinition, joinForward)
...
testJoinForward =
    describe "Tests the joinForward transform"
        [ test "Tests that the example case works as expected" <|
            \_ ->
                Expect.equal (Ok expectedExample) (joinForward defaultListDefinition example)
        ]
